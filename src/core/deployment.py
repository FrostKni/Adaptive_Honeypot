...
class HoneypotDeploymentManager:
    ...
    def _get_environment(self, honeypot_type: HoneypotType, config: Dict) -> Dict[str, str]:
    ...
    def _get_port_mappings(self, honeypot_type: HoneypotType, host_port: int) -> Dict[str, Any]:
    ...
    async def _get_container(self, honeypot_id: str):
        ...
        try:
            containers = self.client.containers.list(
                all=True,
                filters={"label": f"{self.LABEL_ID}={honeypot_id}"}
            )
...
def get_deployment_manager() -> HoneypotDeploymentManager:
